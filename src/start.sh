...
xterm -geometry 80x25+0+87  -T make -e bash -c 'while true ; do make -q || ( git --no-pager diff --color ; git add go.c ; git commit -m "go $(date --iso=s)" ; make --quiet ) ; sleep 1 ; done' &
