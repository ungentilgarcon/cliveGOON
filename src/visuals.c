...
"    middle = middle6;\n"
"  }\n"
"  ivec2 imiddle = ivec2(floor(fly_size * middle * 2.0));\n"
"  int icell = int(floor(float(imiddle.x) / 2.0)) + ((1 + int(floor(float(imiddle.y) / 2.0))) & 1);\n"
"  icell += 1000;\n"
"  icell = int((float(icell)/3.0 - floor(float(icell)/3.0)) * 3.0) % 3;\n"
